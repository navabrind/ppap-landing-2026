...
var RequestInterceptor = class {
  ...
  getStats() {
    ...
    const stats = {
      ...
      cachedRequests: records.filter((r) => r.status === "cached").length,
...
`;
  ...
  getStatusBreakdown(records) {
    const statusCounts = {
      ...
      cached: records.filter((r) => r.status === "cached").length,
    ...
    if (statusCounts.cached > 0) parts.push(`${statusCounts.cached} cached`);
